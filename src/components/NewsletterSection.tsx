...
const NewsletterSection: React.FC = () => {
  ...
  return (
    ...
    >
      {/* Single white container — one fade+slide animation when it enters viewport */}
      <div
        className={`bg-white rounded-2xl px-6 py-10 md:px-10 md:py-14 transition-all duration-700 ease-out-quad ${
      ...
      >
        ...
        <form
          ...
          className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mt-8 max-w-xl"
          noValidate={false}
        >
          ...
          <input
            ...
            className="flex-1 px-4 py-3 rounded-xs border border-primary/70 bg-white text-primary placeholder-primary/50 focus:outline-none focus:border-primary transition-colors duration-200 disabled:opacity-60"
          />
          <button
            type="submit"
            disabled={status === "submitting" || status === "success"}
            className="py-3 px-4 rounded-xs text-primary cursor-pointer border border-primary/70 hover:bg-primary hover:text-white hover:border-primary transition-colors duration-200 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-primary"
          >
            {status === "submitting"
